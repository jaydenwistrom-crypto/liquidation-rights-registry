"""
deploy_aave_liq.py — Deploy AaveRightsLiquidator to Base mainnet.

Writes AAVE_RIGHTS_LIQUIDATOR to .env on success.

Usage:
    venv/bin/python3 deploy_aave_liq.py [--dry-run]
"""
...
abi, bytecode = _load_artifact("AaveRightsLiquidator")

if args.dry_run:
    print(f"[DRY-RUN] AaveRightsLiquidator  bytecode={len(bytecode)//2-1} bytes")
    print(f"[DRY-RUN] constructor arg: _owner={OWNER}")
    sys.exit(0)

confirm = input("\n[?] Deploy AaveRightsLiquidator to Base mainnet? [yes/no] ").strip().lower()
if confirm != "yes":
    sys.exit("Aborted.")

print("\n── Deploying AaveRightsLiquidator ────────────────────────────────────")
addr = _deploy("AaveRightsLiquidator", abi, bytecode, OWNER)
set_key(str(ENV_PATH), "AAVE_RIGHTS_LIQUIDATOR", addr)
print(f"[+] .env: AAVE_RIGHTS_LIQUIDATOR={addr}")
print(f"\n[+] Start executor: venv/bin/python3 aave_rights_executor.py")
